...
class Character:
    ...
    def take_meds(self, item):
        self.medsSlot = item
        self.inventory.append(item)
        print(f"{item.name} был добавлен в инвентарь")
        return True

    def drop_meds(self, item):
        self.medsSlot = False
        self.inventory.remove(item)
        print(f"{item.name} был удален из инвентаря")

    def take_weapon(self, item):
        self.slot = True
        self.damage += item.physDmg + item.magicDmg
        self.speed += item.fireRate
        self.fRange += item.range
        self.gAccuracy += item.accuracy
        self.cAttackPoints = item.fireRate // 5
...
def CaseOpen(character):
    ...
    print(f"Вы нашли {givenDrop.name} в {crateName}")
    ...
    if caseType == "weapon":
        try:
            ...
            print(e)
        if dropChoice == 1:
            character.take_weapon(givenDrop)
        else:
            print("Решив, что данная единица вооружения говно, вы прошли мимо.")
    if caseType == "meds":
        try:
            dropChoice = int(input())
        except ValueError as e:
            print(e)
        if dropChoice == 1:
            character.take_meds(givenDrop)
        else:
            print("Решив что данный медикамент вам не нужен, или непригоден для использования вы его не взяли.")
    if caseType == "armor":
        try:
            dropChoice = int(input())
        except ValueError as e:
